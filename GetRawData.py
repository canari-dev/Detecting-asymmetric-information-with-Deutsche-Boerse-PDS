    ...
    for stk in stocks_list:
        ...
        if stk != 'SX5E':
            ...
            unprocessed_df = unprocessed_df[
                ['Time', 'Date', 'NumberOfContracts', 'MaxPrice', 'MinPrice', 'UnderlyingISIN']]
        ...
        unprocessed_df_opt["CalcDateTime"] = pd.to_datetime(
            unprocessed_df_opt["Date"] + " " + unprocessed_df_opt["Time"])
        ...
        trimed_opt = sorted_by_index_opt[
            ['PriceO', 'ErrorO', 'NumberOfContracts', 'NumberOfTrades', 'MaturityDate', 'StrikePrice', 'PutOrCall',
             'MLEG']]
        trimed_opt['MaturityDate'] = pd.to_datetime(trimed_opt['MaturityDate'], format="%Y%m%d")
        trimed_opt['MaturityDate'] = trimed_opt['MaturityDate'].apply(
            lambda x: x + datetime.timedelta(hours=15, minutes=30))
        trimed_opt['TTM'] = trimed_opt.apply(lambda x: time_between(x.name, x.MaturityDate), axis=1)
